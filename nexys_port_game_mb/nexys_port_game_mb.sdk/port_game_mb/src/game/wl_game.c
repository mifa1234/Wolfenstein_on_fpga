...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in SetupGameLevel (file); \n\r", __FILE__, __LINE__);
    int  x,y;
    int  mapnum;
    word *map;
    word tile;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t- [%s:%d] in DrawPlayBorder (file); \n\r", __FILE__, __LINE__);
	const int px = scaleFactor; // size of one "pixel"
...
{
	//if(XIL_PRINTF_EN == 1) xil_printf("viewsize == %d ingame = %d) \n\r",viewsize,ingame  );
	if(XIL_PRINTF_EN == 1) xil_printf("\t- [%s:%d] in DrawPlayScreen (file); \n\r", __FILE__, __LINE__);
	//xil_printf("\t\tVWB_DrawPicScaledCoord: %d %d \n\r",(screenWidth-scaleFactor*320)/2,screenHeight-scaleFactor*STATUSLINES  );
	//xil_printf("\t\tscreenWidth == %d screenHeight = %d \n\r",screenWidth,screenHeight  );
	//xil_printf("\t\tscaleFactor == %d STATUSLINES = %d \n\r",scaleFactor,STATUSLINES  );


	VWB_DrawPicScaledCoord ((screenWidth-scaleFactor*320)/2,screenHeight-scaleFactor*STATUSLINES,95); //STATUSBARPIC == 95
    DrawPlayBorder ();
...
void ShowActStatus(int pos_pic)
{
	int index = 95; //STATUSBARPIC
    // Draw status bar without borders
    byte *source = grsegs[index];
    int	picnum = index - STARTPICS;
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in ShowActStatus() width = %d, height = %d, destx = %d, desty = %d, picnum = %d, index = %d, pos_pic = %d\n\r", __FILE__, __LINE__, width, height, destx, desty, picnum, index, pos_pic);
...
void GameLoop (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in GameLoop", __FILE__, __LINE__);
    boolean died;
#ifdef MYPROFILE
    clock_t start,end;
#endif

    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] ClearMemory", __FILE__, __LINE__);
    ClearMemory ();



    SETFONTCOLOR(0,15);
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] VW_FadeOut", __FILE__, __LINE__);
    ...
    Delay(100);//pause for start frame

    VW_FadeOut();



    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] DrawPlayScreen", __FILE__, __LINE__);
    DrawPlayScreen ();
    ...
    do
    {
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] DrawLevel", __FILE__, __LINE__);
...
//        DrawLevel ();                     // ADDEDFIX 5 - moved up  Chris Chokan
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] PlayLoop", __FILE__, __LINE__);
        PlayLoop ();
