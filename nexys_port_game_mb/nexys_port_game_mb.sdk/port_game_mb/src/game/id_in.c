...
{
	//xil_printf("[%s:%d] in IN_HandleEvent \n\r", __FILE__, __LINE__);
	int key;
    ...
    {
        case SDL_QUIT:
        	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in IN_HandleEvent: state Quit() \n\r", __FILE__, __LINE__);
            Quit (NULL);
            break;

        case SDL_KEYDOWN:
        	//xil_printf("\t[%s:%d] in IN_HandleEvent: state key_down \n\r", __FILE__, __LINE__);
        ...
        case SDL_KEYUP:
        	//xil_printf("\t[%s:%d] in IN_HandleEvent: state key_up \n\r", __FILE__, __LINE__);
            key = IN_MapKey(key);
...
#endif
        case SDL_TEXTINPUT:
        	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in IN_HandleEvent: state text_input \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_ProcessEvents \n\r", __FILE__, __LINE__);

    SDL_Event event;
    //xil_printf("\t[%s:%d] in IN_ProcessEvents: sizeof(event): %d \n\r", __FILE__, __LINE__, sizeof(event));

    while ((SDL_PollEvent(&event) & 0xFFFF)){
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_WaitEvent \n\r", __FILE__, __LINE__);

	uint32_t count_event = 0;
	uint32_t count_timeout = 1000000;

	while(count_event == 0){
		count_event = Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 7*4);
		count_timeout--;
		if(count_timeout == 0) {
			if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] timeout wait event \n\r", __FILE__, __LINE__);
			return;
		}
	}
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_WaitAndProcessEvents \n\r", __FILE__, __LINE__);
	IN_WaitEvent ();
    IN_ProcessEvents ();
}
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_ReadControl \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_StartAck \n\r", __FILE__, __LINE__);
	int i;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_CheckAck \n\r", __FILE__, __LINE__);
	int i;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_UserInput \n\r", __FILE__, __LINE__);
	longword	lasttime;
	...
	do
	{
		...
		if (IN_CheckAck()){
			...
			if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in IN_UserInput: return true \n\r", __FILE__, __LINE__);
			return true;
		}
