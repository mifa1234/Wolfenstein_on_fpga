...
void ScalePost (void)
{
	//xil_printf("\t\t[%s:%d] in ScalePost() \n\r", __FILE__, __LINE__);
	//uint32_t start_time = Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8);
    ...
    //xil_printf("\t\t\t ScalePost() time: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8) - start_time);
}
...
{
	//xil_printf("\t\t[%s:%d] in HitHorizWall() \n\r", __FILE__, __LINE__);
	//uint32_t start_time =  Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8);
    int wallpic;
    int texture;
   ...
   // xil_printf("\t\t HitHorizWall() time: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8) - start_time);
}
...
void DrawScaleds (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in DrawScaleds() \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in DrawPlayerWeapon() \n\r", __FILE__, __LINE__);
    int shapenum;
...
//
//    xil_printf("\t\t[%s:%d] for from 0 to %d \n\r", __FILE__, __LINE__, viewwidth);
...
=====================
=
= WallRefresh - Optimized version without goto and local copies
=
= Key changes:
= - Removed all goto statements while preserving original logic
= - Work directly with global variables (no local copies)
= - Maintained all pushwall/door special cases
= - Structured the ray tracing loop properly
= - Preserved original algorithm behavior
=
=====================
*/
...
void WallRefresh (void)
{
    if(XIL_PRINTF_EN == 1)
    	xil_printf("\t[%s:%d] in WallRefresh() \n\r", __FILE__, __LINE__);
    //Xil_Out32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8, 0);

    //xil_printf("\tstart time: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));
    ...
    // xil_printf("\t\t[%s:%d] for from 0 to %d \n\r", __FILE__, __LINE__, viewwidth);
    //xil_printf("\t delta time pixx: ");
    for (pixx = 0; pixx < viewwidth; pixx++)
    {
//    	cur_time1 = Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8);
//    	xil_printf(" %d", cur_time1 - old_time1);
//    	old_time1 = cur_time1;
        ...
        //xil_printf("\t if(BIT_WALL) time: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));

        //
        // Ray tracing loop - replacement for goto-based logic
        //
        hit_wall = false;
        check_vertical = true;
        ...
        while (!hit_wall)
        {
        	//xil_printf("\t while (!hit_wall) time: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));
            if (check_vertical)
            {
                //
                // check intersections with vertical walls
...
//            horizentry:
    ...
    //if(XIL_PRINTF_EN == 1) xil_printf("\tall time WallRefresh(): %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));
}
...
//
	if(XIL_PRINTF_EN == 1)
		xil_printf("[%s:%d] in ThreeDRefresh() \n\r", __FILE__, __LINE__);
...
//    if(Keyboard[sc_Alt]){
//    	if(glbl_pic >= 157) glbl_pic = STARTPICS;
//    	else glbl_pic++;
//    }
    if(Keyboard[sc_Tab] && viewsize == 21 && gamestate.weapon != -1){

    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] call ShowActStatus() \n\r", __FILE__, __LINE__);
        ShowActStatus(glbl_pic);
    }
    //Xil_DCacheFlush();
...
//
    ...
    //Xil_DCacheFlush();
   // Xil_DCacheFlushRange(screenBuffer, 640*240*2);
