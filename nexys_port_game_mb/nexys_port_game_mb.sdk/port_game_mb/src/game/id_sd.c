...
// Заглушки для функций SDL_mixer
void Mix_OpenAudio(int frequency, Uint16 format, int channels, int chunksize) {}
void Mix_CloseAudio(void) {}
Mix_Chunk* Mix_LoadWAV(const char* file) { return NULL; }
...
int Mix_AllocateChannels(int numchannels) { return 0; }
...
// ================== ЗАГЛУШКИ ДЛЯ SDL_mixer и FM OPL ==================
#define MIX_CHANNELS 16

// Заглушки для функций SDL_mixer
int Mix_GroupAvailable(int group) { return -1; }
int Mix_GroupOldest(int group) { return -1; }
void Mix_SetPanning(int channel, Uint8 left, Uint8 right) {}
Mix_Chunk* Mix_LoadWAV_RW(SDL_RWops* src, int freesrc) { return NULL; }
const char* Mix_GetError(void) { return "Sound disabled"; }
int Mix_OpenAudioDevice(int frequency, Uint16 format, int channels,
                       int chunksize, const char* device, int allowed_changes) { return -1; }
void Mix_ReserveChannels(int num) {}
int Mix_GroupChannels(int from, int to, int group) { return 0; }
...
// Заглушки для FM OPL функций
void YM3812Write(int chip, int a, int v) {}
int YM3812Init(int num, int baseclock, int rate) { return 0; }
void YM3812UpdateOne(int chip, Sint16 *buffer, int length) {}
...
//#include "audiosod.h"
//
//// Заглушка для SD_PlaySound
//boolean SD_PlaySound(soundnames sound)
//{
//	xil_printf("by_pass SD_PlaySound");
//    return false;
//}
//
//// Заглушка для SD_StopSound
//void SD_StopSound(void)
//{
//	xil_printf("by_pass SD_StopSound");
//}
//
//// Заглушка для SD_StartMusic
//boolean SD_StartMusic(int music)
//{
//	xil_printf("by_pass SD_StartMusic");
//    return false;
//}
//
//// Заглушка для SD_MusicPlaying
//boolean  SD_MusicPlaying (void)
//{
//	xil_printf("by_pass SD_MusicPlaying\n\r");
//    return false;
//}
//
//// Заглушка для SD_SetMusicMode
//void SD_SetMusicMode(int mode)
//{
//	xil_printf("by_pass SD_SetMusicMode\n\r");
//}
//
//// Заглушка для SD_SetSoundMode
//void SD_SetSoundMode(int mode)
//{
//	xil_printf("by_pass SD_SetSoundMode\n\r");
//}
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t- [%s:%d] in SD_StopDigitized (file); \n\r", __FILE__, __LINE__);
    DigiPlaying = false;
    DigiNumber = 0;
    DigiPriority = 0;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SD_PrepareSound \n\r", __FILE__, __LINE__);
...
SD_Startup(void)
{
    ...
    if (SD_Started){
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] SAUND ALREADY started//// HOW???? (file); \n\r", __FILE__, __LINE__);
        return;
    }

    // Простая заглушка - выделяем минимальный буфер если нужен
    static uint8_t audio_buffer[1024]; // Минимальный буфер-заглушка

    // Устанавливаем флаги в false - звука нет
    AdLibPresent = false;
    ...
    // Устанавливаем режимы "выключено"
    SD_SetSoundMode(sdm_Off);
    SD_SetMusicMode(smm_Off);

    SD_Started = true;

    if(XIL_PRINTF_EN == 1) xil_printf("Sound system: DISABLED (MicroBlaze port)\n\r");
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] SD_Startup end (file); \n\r", __FILE__, __LINE__);
    return;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ERROR inside SD_PlaySound()", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SD_StartMusic", __FILE__, __LINE__);
	SD_MusicOff();
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SD_ContinueMusic", __FILE__, __LINE__);
    int i;
