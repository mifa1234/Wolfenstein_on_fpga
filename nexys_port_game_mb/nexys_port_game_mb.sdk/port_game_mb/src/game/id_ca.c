...
static void CAL_HuffExpand(byte *source, byte *dest, int32_t length, huffnode *hufftable)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d]  inside CAL_HuffExpand. THIS FUNCTION need OPTimization!!! letter\n\r", __FILE__, __LINE__);
	byte *end;
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d]  while(1) end=dest+length(%d): %d (%x)\n\r", __FILE__, __LINE__, length, end, end);
    while(1)
    {
    	cnt_iter++;

    	if(next_step == written) {  //if((cnt_iter&0x1FF) == 0) {
    		if(XIL_PRINTF_EN == 1) xil_printf(" %d/%d, ", written, length);
    		cnt_step++;
...
#define MASKBLOCK       128
    ...
    if((expanded > 0) && (expanded < 14000000) ){
    	...
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in CAL_ExpandGrChunk \n\r", __FILE__, __LINE__);
    	CAL_HuffExpand((byte *) source, grsegs[chunk], expanded, grhuffman);
    } else {
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in CAL_ExpandGrChunk. ERROR SafeMalloc(expanded) = %x\n\r", __FILE__, __LINE__, expanded);
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d]\t\t\t this NEED WORK!!!!\n\r", __FILE__, __LINE__);
    }
}
...
void CAL_DeplaneGrChunk (int chunk)
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in CAL_DeplaneGrChunk ->", __FILE__, __LINE__);
    ...
    if (chunk == STARTTILE8)
    {
    	...
    	if(XIL_PRINTF_EN == 1) xil_printf("chunk == STARTTILE8 NUMTILE8: %d\n\r", __FILE__, __LINE__, NUMTILE8);
    ...
    else
    {
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("chunk != STARTTILE8. call VL_DePlaneVGA\n\r", __FILE__, __LINE__);
...
void CA_CacheGrChunks (FILE *grfile)
{
    ...
    int32_t *source;
    int     chunk,next;
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in CA_CacheGrChunks for iter from %d to %d\n\r", __FILE__, __LINE__, STRUCTPIC + 1, NUMCHUNKS);
    ...
    for (chunk = STRUCTPIC + 1; chunk < NUMCHUNKS; chunk++)
    {
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("\t\t_[%s:%d] call CA_CacheGrChunks %d/%d\n\r", __FILE__, __LINE__, chunk, NUMCHUNKS);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in CA_CacheMap", __FILE__, __LINE__);
    FILE     *file;
    char     fname[13];
    int32_t  pos,compressed;
    int      plane;
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] for iter plane: %d", __FILE__, __LINE__, MAPPLANES);
