...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in ReadConfig \n\r", __FILE__, __LINE__);
    byte  sd;
    byte  sm;
    byte sds;
    FILE *file;
...
noconfig:
        ...
        viewsize = 19; //20; //19;  //21                        // start with a good size
        mouseadjustment=5;
    }
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in NewGame", __FILE__, __LINE__);
...
void ShutdownId (void)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in ShutdownId. BAD   BAD  BAD!!!! \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in BuildTables \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SetupWalls \n\r", __FILE__, __LINE__);
...
{
//	uint32_t *base_addr_frame_buffer = XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR;
//	if(screen->pixels != NULL) {
//		base_addr_frame_buffer[0] = &screen->pixels[0];
//	} else {
//		if(XIL_PRINTF_EN == 1) xil_printf("SignonScreen_0: screen->pixels == NULL \n\r");
//	}

	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
	if(XIL_PRINTF_EN == 1) xil_printf("VL_SetVGAPlaneMode \n\r");
	VL_SetVGAPlaneMode ();

//	if(screen->pixels != NULL) {
//    	base_addr_frame_buffer[0] = &screen->pixels[0];
//    } else {
//    	xil_printf("SignonScreen_1: screen->pixels == NULL \n\r");
//    }

	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("VL_MemToScreen \n\r");
    VL_MemToScreen (signon,320,200,0,0);
//    if(screen->pixels != NULL) {
//    	base_addr_frame_buffer[0] = &screen->pixels[0];
//    } else {
//    	xil_printf("SignonScreen_2: screen->pixels == NULL \n\r");
//    }
}
...
void FinishSignon (void)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in FinishSignon \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in InitDigiMap \n\r", __FILE__, __LINE__);

	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] TEMP off SOUND \n\r", __FILE__, __LINE__);

	memset(DigiMap, 0, LASTSOUND);
	memset(DigiChannel, 0, STARTMUSIC - STARTDIGISOUNDS);
    ...
    {
    	if(XIL_PRINTF_EN == 1) xil_printf("\tmap=%d \n\r", map);
        DigiMap[map[0]] = map[1];
...
void DoJukebox(void)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in DoJukebox \n\r", __FILE__, __LINE__);
...
static void InitGame()
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("SignonScreen \n\r");
    SignonScreen ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("VW_UpdateScreen \n\r");
	VW_UpdateScreen();

	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
	if(XIL_PRINTF_EN == 1) xil_printf("VH_Startup \n\r");
    VH_Startup ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("IN_Startup \n\r");
    IN_Startup ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("PM_Startup \n\r");
    PM_Startup ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("SD_Startup \n\r");
    SD_Startup ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("CA_Startup \n\r");
    CA_Startup ();

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("US_Startup \n\r");
    US_Startup ();
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("BEFORE: ReadConfig (). send 1\n\r");
    int byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("BEFORE: SetupSaveGames (). send 1\n\r");
    byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("BEFORE: IN_ProcessEvents (). send 1\n\r");
    byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("BEFORE: if (Keyboard[sc_M]). send 1\n\r");
    byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
    ...
    if (Keyboard[sc_M])
    {
        ...
        xil_printf("BEFORE: IntroScreen (). send 1\n\r");
        byte = '1';
        while(byte != '1'){
        	byte = inbyte();
    		xil_printf("rcv: %c\n\r",byte);
...
//
    ...
    xil_printf("BEFORE: BuildTables (). send 1\n\r");
    byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
...
//
    ...
    xil_printf("BEFORE: NewViewSize (). send 1\n\r");
    byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
...
//
    ...
    if(!didjukebox)
        ...
        xil_printf("BEFORE: FinishSignon (). send 1\n\r");
        byte = '1';
        while(byte != '1'){
        	byte = inbyte();
    		xil_printf("rcv: %c\n\r",byte);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in NewViewSize \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in DemoLoop", __FILE__, __LINE__);
    int LastDemo = 0;

//
// check for launch from ted
//
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in DemoLoop: STATE 0", __FILE__, __LINE__);
    if (param_tedlevel != -1)
    {
        param_nowait = true;
        if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] EnableEndGameMenuItem", __FILE__, __LINE__);
        EnableEndGameMenuItem();
        if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] NewGame", __FILE__, __LINE__);
...
#endif
        if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] GameLoop", __FILE__, __LINE__);
...
// main game cycle
//
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in DemoLoop: STATE 1", __FILE__, __LINE__);
#ifndef DEMOTEST
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in DemoLoop: STATE 2", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in DemoLoop before while(1) \n\r", __FILE__, __LINE__);
    while (1)
    {
        while (!param_nowait)
        {
        	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in DemoLoop: while (!param_nowait) \n\r", __FILE__, __LINE__);
//
// title page
//
#ifndef DEMOTEST
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in DemoLoop: STATE end", __FILE__, __LINE__);
}
...
//#endif
	xil_printf("Set game parameters... ");
	// Основные параметры графики
	screenWidth = 320;        // Ширина экрана (можно увеличить позже)
	screenHeight = 240;       // Высота экрана
	screenBits = 16;          // 16-битный цвет (RGB565)
	fullscreen = 0;           // Не используем полноэкранный режим в классическом понимании

	// Параметры геймплея
	param_difficulty = 2;     // Normal difficulty (1-3)
	param_nowait = 1;         // Не ждать вертикальной синхронизации (важно для bare-metal)
	param_mission = 0;        // Базовая миссия

	// Параметры звука (пока можно отключить)
	param_samplerate = 0;     // Частота дискретизации (0 - нет звука)
	param_audiobuffer = 0;    // Размер аудиобуфера

	// Специфические параметры
	param_debugmode = 0;      // Отключить debug-режим
	param_tedlevel = 0;       // Уровень для редактора (не используем)
	configdir[0] = '\0';      // Путь к конфигам (не используем)

	xil_printf("OK\n\r");

	xil_printf("[%s:%d] ", __FILE__, __LINE__);
	xil_printf("Check For Episodes... ");
    CheckForEpisodes();
    xil_printf("OK\n\r");

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("Init Game... ");
    InitGame();
    if(XIL_PRINTF_EN == 1) xil_printf("OK\n\r");

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    if(XIL_PRINTF_EN == 1) xil_printf("Demo Loops... ");
    if(XIL_PRINTF_EN == 1) xil_printf("BEFORE: DemoLoop(). send 1\n\r");
    int byte = '1';
    while(byte != '1'){
    	byte = inbyte();
		xil_printf("rcv: %c\n\r",byte);
    ...
    DemoLoop();
    if(XIL_PRINTF_EN == 1) xil_printf("OK\n\r"); //VL_MemToScreenScaledCoord  gamemaps

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    Quit("Demo loop exited???"); //---- while ((uintptr_t)dstp1 & 0x3) {
    return 1;
}
