...
void StatusDrawPic(unsigned x, unsigned y, unsigned picnum)
{
    ...
    // Ѕазовые координаты статус бара
    int base_x = (screenWidth - scaleFactor * 320) / 2;
    int base_y = screenHeight - scaleFactor * STATUSLINES;

    // ѕреобразуем относительные координаты в абсолютные
    int scx = base_x + x * scaleFactor;
    int scy = base_y + y * scaleFactor;

    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in StatusDrawPic. base_x = %d, base_y = %d (screenWidth:%d,screenHeight:%d)\n\r", __FILE__, __LINE__, base_x,base_y, screenWidth,screenHeight );

    VWB_DrawPicScaledCoord(scx, scy, picnum);
}
...
void DrawFace (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in DrawFace \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) {
    	xil_printf("\t\t error. viewsize == 21 && ingame");
    	return;
    }
...
static void LatchNumber (int x, int y, unsigned width, int32_t number)
{
	...
	for(i = width; i >0; i--)
	{
		...
		StatusDrawPic (x + i*8,y,108 + num_a_mod); // 108 - is  zero
	}
...
//
//    while (length<width)//clear old pic
//    {
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in DrawHealth \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) {
    	xil_printf("\t\t error \n\r");
    	return;
    }
    LatchNumber (160,16,3,gamestate.health);
}
...
void DrawLevel (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawLevel (file); \n\r", __FILE__, __LINE__);
...
#endif
        LatchNumber (10,16,2,gamestate.mapon+1);
}
...
void DrawLives (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawLives (file); \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) {
    	xil_printf("\t\t error \n\r");
    	return;
    }
    LatchNumber (106,16,1,gamestate.lives);
}
...
void DrawScore (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawScore (file); \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) return;
    LatchNumber (38,16,6,gamestate.score);
}
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawWeapon (file); \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) return;
    //StatusDrawPic (32,8,KNIFEPIC+gamestate.weapon);
    StatusDrawPic (250,8,100+gamestate.weapon);
}
...
void DrawKeys (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawKeys (file); \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) return;

    if (gamestate.keys & 1)
        StatusDrawPic (240,4,105); //GOLDKEYPIC
    else
        StatusDrawPic (240,4,104);//NOKEYPIC

    if (gamestate.keys & 2)
        StatusDrawPic (240,20,106); //SILVERKEYPIC
    else
        StatusDrawPic (240,20,104); //NOKEYPIC
...
void DrawAmmo (void)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in DrawAmmo (file); \n\r", __FILE__, __LINE__);
    if(viewsize == 21 && ingame) return;
    LatchNumber (198,16,2,gamestate.ammo);
}
