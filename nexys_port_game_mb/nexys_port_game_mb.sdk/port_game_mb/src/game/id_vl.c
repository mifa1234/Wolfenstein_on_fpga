...
int16_t  screenWidth = 320;
int16_t  screenHeight = 240;
int      screenBits = 16;      // use "best" color depth according to libSDL
#endif
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in VL_SetVGAPlaneMode ", __FILE__, __LINE__);
	int i;
    uint32_t a,r,g,b;
...
#endif
    if(XIL_PRINTF_EN == 1) xil_printf("-SDL_CreateWindow, fullscreen: %d\n\r", fullscreen);
    window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screenWidth, screenHeight, (fullscreen ? SDL_WINDOW_FULLSCREEN : 0) );  // (fullscreen ? SDL_WINDOW_FULLSCREEN : 0) | SDL_WINDOW_OPENGL

    SDL_PixelFormatEnumToMasks (SDL_PIXELFORMAT_ARGB8888,&screenBits,&r,&g,&b,&a);


//    if(XIL_PRINTF_EN == 1)
    	xil_printf(" [%s:%d] -SDL_CreateRGBSurface \n\r", __FILE__, __LINE__);

    screenBits = 16;
    r = 0x7C00; // Маска для красного в RGB555
    g = 0x03E0; // Маска для зеленого в RGB555
    b = 0x001F; // Маска для синего в RGB555
    a = 0;      // Без альфа-канала
    screen = SDL_CreateRGBSurface(0, screenWidth, screenHeight, screenBits, r, g, b, a);

    Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4, &(screen->pixels[0]));
    Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+8, &(screen->pixels_double_buff[0]));

    // И после создания surface установите прямой указатель на ваш буфер:
    //screen->pixels = XPAR_MIG_7SERIES_0_BASEADDR + 0x5000000;
    screen->pitch = screenWidth * 2; // Обновляем pitch для 16-битного формата

//    if(XIL_PRINTF_EN == 1) xil_printf("-SDL_CreateRGBSurface \n\r");
//    screen = SDL_CreateRGBSurface(0,screenWidth,screenHeight,screenBits,r,g,b,a);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] -SDL_CreateRenderer", __FILE__, __LINE__);
    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0");

    if(XIL_PRINTF_EN == 1) xil_printf("\t\t [%s:%d] SDL_ShowCursor(SDL_DISABLE)  - call was off!!!", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t-- [%s:%d] in VL_FadeOut (file); \n\r", __FILE__, __LINE__);
	int		    i,j,orig,delta;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in VL_BarScaledCoord \n\r", __FILE__, __LINE__);
	byte *dest;
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t\t[%s:%d] in VL_DePlaneVGA\n\r", __FILE__, __LINE__);
    int  x,y,plane;
    word size,pwidth;
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t\t[%s:%d] for iter, num iter: %d \n\r", __FILE__, __LINE__, 4*height*pwidth);
...
//
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t\t  [%s:%d] after_ memcpy (source,temp,size);\n\r", __FILE__, __LINE__);
...
void VL_MemToScreenScaledCoord (byte *source, int width, int height, int destx, int desty)
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in --inside VL_MemToScreenScaledCoord (w=%d, h=%d, destx=%d, desty=%d) \n\r", __FILE__, __LINE__, width, height, destx, desty);

//    assert(destx >= 0 && destx + width * scaleFactor <= screenWidth
//            && desty >= 0 && desty + height * scaleFactor <= screenHeight
//            && "VL_MemToScreenScaledCoord: Destination rectangle out of bounds!");
    if (destx < 0 || desty < 0 ||
        destx + width * scaleFactor > screenWidth ||
        desty + height * scaleFactor > screenHeight)
    {
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
        if(XIL_PRINTF_EN == 1) xil_printf("VL_MemToScreenScaledCoord: skipped out-of-bounds draw at (%d,%d), size %dx%d (scaled %dx%d)\n\r",
                   destx, desty, width, height, width*scaleFactor, height*scaleFactor);
        //return;
    }

    dest = VL_LockSurface(screenBuffer);
    if(dest == NULL) {
    	xil_printf("--Eror if(dest == NULL) {\n\r");
    	return;
    }


    //xil_printf("--inside VL_MemToScreenScaledCoord :  for(j = 0,... to %d iter\n\r", (int)(height)*(width)*scaleFactor*scaleFactor);
    //xil_printf("id_vl.c:672 height: %d; width: %d; scaleFactor: %d\n\r", height,width,scaleFactor);


    // Правильный способ работы с ylookup
    for (j = 0; j < height; j++)
    {
        // ylookup[j + desty] уже содержит указатель на начало строки (j + desty)
        // Поэтому просто прибавляем destx для смещения по горизонтали
        byte *dest_row = dest + ylookup[j + desty] + destx;

        byte *src_row = source + (j * width);

        // Копируем строку, но не выходя за границы экрана
        int copy_width = width;
        if (destx + copy_width > screenWidth) {
            copy_width = screenWidth - destx;
        }

        if (copy_width > 0 && j + desty < screenHeight) {
            memcpy(dest_row, src_row, copy_width);
        }
    }

    if(XIL_PRINTF_EN == 1) xil_printf("--VL_UnlockSurface\n\r");
    VL_UnlockSurface(screenBuffer);
}
