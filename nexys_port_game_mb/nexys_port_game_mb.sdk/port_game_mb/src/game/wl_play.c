...
//uint16_t second_buffer[640*480];
/*
=============================================================================
...
void PollControls (void)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in PollControls \n\r", __FILE__, __LINE__);
...
void CheckKeys (void)
{
    ...
    if (Keyboard[sc_H] && Keyboard[sc_A])
    {
        ClearMemory ();
        ClearSplitVWB ();
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in InitRedShifts \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) {
		xil_printf("\t-- [%s:%d] in PlayLoop (file); \n\r", __FILE__, __LINE__);
		xil_printf("\t-- [%s:%d] SECRET CHEAT CODE: 'MLI' \n\r", __FILE__, __LINE__);
	}
    objtype *obj;
   ...
   // base_addr_frame_buffer[0] = &second_buffer[0];

    memset(Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4), 0x33, 640*480*2);
    memset(Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+8), 0x77, 640*480*2);
...
//    	Xil_DCacheFlushRange(screenBuffer->pixels, 640*240*2);
    	//Xil_DCacheFlush();
...
//
        ...
        //while(Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8) < 24000){
        	//Xil_DCacheFlush();
        //}

        //Xil_DCacheFlush();
        //Xil_DCacheInvalidateRange(Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4), 640*480*2);
        //memcpy( Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+8),  Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4) , 640*480*1) ;

        //screen->ch_src_pixels_addr = 1;
//        Xil_DCacheFlush();
        //Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + 0, Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+8));
...
//        Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+8, &(screen->pixels_double_buff[0]));
        ...
        if(XIL_PRINTF_TIME_EN == 1) xil_printf("total time for game loop: %d \n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));
        if(XIL_PRINTF_TIME_EN == 2) xil_printf("%d\n\r", Xil_In32(XPAR_ANY_REG_SPACE_TIMER_BASEADDR+8));
