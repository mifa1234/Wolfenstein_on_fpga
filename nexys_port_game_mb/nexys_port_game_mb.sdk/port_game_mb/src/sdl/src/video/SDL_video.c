...
#define CHECK_WINDOW_MAGIC(window, retval) \
	xil_printf("\t[%s:%d] CHECK_WINDOW_MAGIC ", __FILE__, __LINE__); \
    if (!_this) { \
...
{
	xil_printf("[%s:%d] in SDL_CreateWindowTexture \n\r", __FILE__, __LINE__);
    ...
    if (!data) {
        ...
        if (specific_accelerated_renderer) {
            ...
            if (!renderer || (SDL_GetRendererInfo(renderer, &info) == -1)) {
            	xil_printf("[%s:%d] call SDL_DestroyRenderer \n\r", __FILE__, __LINE__);
...
{
	xil_printf("[%s:%d] in SDL_UpdateWindowTexture \n\r", __FILE__, __LINE__);
...
{
	xil_printf("[%s:%d] in SDL_DestroyWindowTexture \n\r", __FILE__, __LINE__);
	SDL_WindowTextureData *data;
...
SDL_CreateWindow(const char *title, int x, int y, int w, int h, Uint32 flags)
{
    ...
    xil_printf("- in SDL_CreateWindow \n\r");
    xil_printf("-- STATE_W 0 \n\r");

    if ((((flags & SDL_WINDOW_UTILITY) != 0) + ((flags & SDL_WINDOW_TOOLTIP) != 0) + ((flags & SDL_WINDOW_POPUP_MENU) != 0)) > 1) {
    	xil_printf("[%s:%d] SDL_SetError: ", __FILE__, __LINE__);
    ...
    xil_printf("-- STATE_W 6 \n\r");
    window = (SDL_Window *)SDL_calloc(1, sizeof(*window));
    if (!window) {
    	xil_printf("-- STATE_W 6.1 \n\r");
        SDL_OutOfMemory();
        return NULL;
    }
    ...
    xil_printf("-- STATE_W 9 \n\r");
    if (_this->windows) {
        _this->windows->prev = window;
    }
    _this->windows = window;

    xil_printf("-- STATE_W 9.1 \n\r");
    if (_this->CreateSDLWindow && _this->CreateSDLWindow(_this, window) < 0) {
    	xil_printf("-- STATE_W 9.1.1 \n\r");
        SDL_DestroyWindow(window);
        return NULL;
    }

    xil_printf("-- STATE_W 9.2 \n\r");
    ...
    */
    ...
    xil_printf("-- STATE_W 10 \n\r");
    if (title) {
        SDL_SetWindowTitle(window, title);
    }
    SDL_FinishWindowCreation(window, flags);

    xil_printf("-- STATE_W 11 \n\r");
...
{
	xil_printf("\t[%s:%d] SDL_DestroyWindow ", __FILE__, __LINE__);
    SDL_VideoDisplay *display;
