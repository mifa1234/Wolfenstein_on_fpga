...
{
//	if(XIL_PRINTF_EN == 1)
		xil_printf("[%s:%d] in SDL_CreateRGBSurfaceWithFormat() \n\r", __FILE__, __LINE__);
    Sint64 pitch;
    SDL_Surface *surface;
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("---SDL_CalculatePitch \n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("---SDL_calloc \n\r");
    surface = (SDL_Surface *) SDL_calloc(1, sizeof(*surface));
    if (surface == NULL) {
    	if(XIL_PRINTF_EN == 1) xil_printf("---if (surface == NULL) { ");
        SDL_OutOfMemory();
        if(XIL_PRINTF_EN == 1) xil_printf("...SDL_OutOfMemory \n\r");
        return NULL;
    }

    if(XIL_PRINTF_EN == 1) xil_printf("---SDL_AllocFormat(format); \n\r");
    surface->format = SDL_AllocFormat(format);
    if (!surface->format) {
    	if(XIL_PRINTF_EN == 1) xil_printf("---if (surface == NULL) { ");
    	SDL_FreeSurface(surface);
        if(XIL_PRINTF_EN == 1) xil_printf("...SDL_FreeSurface(surface); \n\r");
        return NULL;
    }
    surface->w = width;
    surface->h = height;
    ...
    if (SDL_ISPIXELFORMAT_INDEXED(surface->format->format)) {
    	if(XIL_PRINTF_EN == 1) xil_printf("---if (SDL_ISPIXELFORMAT_INDEXED(surface->format->format)) \n\r");
    	SDL_Palette *palette =
    ...
    if (surface->w && surface->h) {
    	if(XIL_PRINTF_EN == 1) xil_printf("---if (surface->w && surface->h) {) \n\r");
        /* Assumptions checked in surface_size_assumptions assert above */
        Sint64 size = ((Sint64)surface->h * surface->pitch);
        if (size < 0 || size > SDL_MAX_SINT32) {
        	if(XIL_PRINTF_EN == 1) xil_printf("---if (size < 0 || size > SDL_MAX_SINT32) {\n\r");
            /* Overflow... */
            SDL_FreeSurface(surface);
            SDL_OutOfMemory();
            return NULL;
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("---SDL_SIMDAlloc\n\r");
        surface->pixels = SDL_SIMDAlloc((size_t)640*480*2); //((size_t)size);
        surface->pixels_double_buff = SDL_SIMDAlloc((size_t)640*480*2);
        //surface->pixels = (void *)(XPAR_MIG_7SERIES_0_BASEADDR + 0x1000000);
        if (!surface->pixels) {
        	if(XIL_PRINTF_EN == 1) xil_printf("---if (!surface->pixels) {\n\r");
            SDL_FreeSurface(surface);
            SDL_OutOfMemory();
            return NULL;
        }
        ...
        if(XIL_PRINTF_EN == 1) xil_printf("---SDL_memset %d (%x) %d %d\n\r", surface->pixels, surface->pixels, surface->h , surface->pitch);
        SDL_memset(surface->pixels, 0, surface->h * surface->pitch);
    }

    /* Allocate an empty mapping */
    if(XIL_PRINTF_EN == 1) xil_printf("---SDL_AllocBlitMap \n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("---if (surface->format->Amask) { \n\r");
                     ...
                     Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("--SDL_MasksToPixelFormatEnum \n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("-- [%s:%d] callSDL_CreateRGBSurfaceWithFormat() \n\r", __FILE__, __LINE__);
              ...
              SDL_Surface * dst, SDL_Rect * dstrect)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("SDL_surface.c:643 SDL_BlitSurface: in SDL_LowerBlit \n\r");
	if ((src->map->dst != dst) ||
        (dst->format->palette &&
...
/*              dst->map->info.flags, src->map->blit); */
	...
	if(XIL_PRINTF_EN == 1) xil_printf("SDL_surface.c:658  SDL_LowerBlit: src->map->blit(src, srcrect, dst, dstrect \n\r");
...
SDL_UpperBlit(SDL_Surface * src, const SDL_Rect * srcrect,
              SDL_Surface * dst, SDL_Rect * dstrect)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t%s:%d in SDL_UpperBlit \n\r", __FILE__, __LINE__);
    SDL_Rect fulldst;
    int srcx, srcy, w, h;
    ...
    if (srcrect) {
    	if(XIL_PRINTF_EN == 1) xil_printf("\t  %s:%d in  if (srcrect) { \n\r", __FILE__, __LINE__);
        int maxw, maxh;
    ...
    if (src->map->info.flags & SDL_COPY_NEAREST) {
    	if(XIL_PRINTF_EN == 1) xil_printf("\t  %s:%d in  if (src->map->info.flags & SDL_COPY_NEAREST) { \n\r", __FILE__, __LINE__);
        src->map->info.flags &= ~SDL_COPY_NEAREST;
        if(XIL_PRINTF_EN == 1) xil_printf("SDL_BlitSurface: SDL_InvalidateMap \n\r");
    ...
    if (w > 0 && h > 0) {
        ...
        sr.w = dstrect->w = w;
        sr.h = dstrect->h = h;
        if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] SDL_BlitSurface: SDL_LowerBlit \n\r", __FILE__, __LINE__);
