...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_memset \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("PASS---- while (len--) { len = %d\n\r", len);
//    while (len--) {
//        *dstp4++ = value4;
//
//        if((len&0xff) == 0) xil_printf("l:%d\n\r", len);
//    }
    xil_printf("---- END while (len--) { len = %d\n\r", len);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] CHANGE SDL_memcpy()", __FILE__, __LINE__);
