...
void *SDL_calloc(size_t nmemb, size_t size)
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t SDL_calloc: s_mem.calloc_func(nmemb, size) = (%d,%d); \n\r", nmemb, size);
    mem = s_mem.calloc_func(nmemb, size);
    if (mem) {
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] ", __FILE__, __LINE__);
    	if(XIL_PRINTF_EN == 1) xil_printf("\t\t SDL_calloc: SDL_AtomicIncRef size: %d ", sizeof(mem));
        SDL_AtomicIncRef(&s_mem.num_allocations);
        if(XIL_PRINTF_EN == 1) xil_printf(" end\n\r");
    }
    return mem;
}
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t SDL_malloc.c:5418 SDL_realloc ");
    void *mem;
