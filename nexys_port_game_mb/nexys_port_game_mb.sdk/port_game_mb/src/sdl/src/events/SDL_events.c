...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_PeepEventsInternal \n\r", __FILE__, __LINE__);
    int i, used, sentinels_expected = 0;

    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_PeepEventsInternal: state 0 \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_PeepEventsInternal: state 1 \n\r", __FILE__, __LINE__);
    ...
    if (!SDL_EventQ.lock || SDL_LockMutex(SDL_EventQ.lock) == 0) {
        if (action == SDL_ADDEVENT) {
            ...
            if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_PeepEventsInternal: state 3 \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_PeepEventsInternal: state 4 \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in SDL_PumpEventsInternal\n\r", __FILE__, __LINE__);
    SDL_VideoDevice *_this = SDL_GetVideoDevice();
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in SDL_ReleaseAutoReleaseKeys: xxx\n\r", __FILE__, __LINE__);
...
{
	//xil_printf("\t\t[%s:%d] in SDL_PollEvent", __FILE__, __LINE__);

	uint32_t count_event;

	count_event = Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 7*4);

	if((count_event & 0xFFFF) > 0){

	    // Устанавливаем тип события (главное поле union)
		//вызываем его первым чтобы остальные поля были вычитаны из фифо
	    event->type = Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 0x0);

	    // Заполняем поля для клавиатуры
	    event->key.timestamp = SDL_GetTicks();  // Обязательно! В миллисекундах
	    event->key.windowID = 0;                 // ID окна (0 для основного)
	    event->key.state =  Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 0x4);
	    event->key.repeat = 0;                   // Драйвер НЕ генерирует repeat

	    // Заполняем keysym
	    event->key.keysym.scancode =  Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 0x8);   // Физический код клавиши
	    event->key.keysym.sym =  Xil_In32(XPAR_KEY_DRIVER_PB_0_S00_AXI_BASEADDR + 0xC); // Виртуальный код
	    event->key.keysym.mod = KMOD_NONE;       // Модификаторы (не используем)
	    event->key.keysym.unused = 0;            // Зарезервировано

	    // Поля padding - уже нули от memset
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_WaitEventTimeout \n\r", __FILE__, __LINE__);
    SDL_VideoDevice *_this = SDL_GetVideoDevice();
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 0 \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 1 \n\r", __FILE__, __LINE__);
    /* First check for existing events */
    result = SDL_PeepEventsInternal(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT, include_sentinel);
    if (result < 0) {
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_PeepEventsInternal: result %d \n\r", __FILE__, __LINE__, result);
        return 0;
    }

    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 2 \n\r", __FILE__, __LINE__);
    if (include_sentinel) {
        if (event) {
            if (event->type == SDL_POLLSENTINEL) {
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 3 \n\r", __FILE__, __LINE__);
    if (result == 0) {
        if (timeout == 0) {
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 4 \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 5 \n\r", __FILE__, __LINE__);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state 6 \n\r", __FILE__, __LINE__);
    for (;;) {
        SDL_PumpEventsInternal(SDL_TRUE);
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in SDL_WaitEventTimeout: state END \n\r", __FILE__, __LINE__);
}

int SDL_PushEvent(SDL_Event * event)
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_PushEvent FIX MEEE!!!! \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in SDL_EventState\n\r", __FILE__, __LINE__);
