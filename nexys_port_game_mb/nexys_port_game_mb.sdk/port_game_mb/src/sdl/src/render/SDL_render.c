...
#define CHECK_RENDERER_MAGIC(renderer, retval) \
    if (!renderer || renderer->magic != &renderer_magic) { \
    	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d]   SDL_RenderPresent: in CHECK_RENDERER_MAGIC !!!ERROR!!!\n\r", __FILE__, __LINE__); \
...
#define CHECK_TEXTURE_MAGIC(texture, retval) \
    if (!texture || texture->magic != &texture_magic) { \
    	if(XIL_PRINTF_EN == 1) xil_printf("\t%s:%d in CHECK_TEXTURE_MAGIC !!!ERROR!!!\n\r", __FILE__, __LINE__); \
...
//#define CHECK_RENDERER_MAGIC(renderer, retval) \
//    if (!renderer || renderer->magic != &renderer_magic) { \
//    	xil_printf("[%s:%d]   SDL_RenderPresent: in CHECK_RENDERER_MAGIC !!!ERROR!!!\n\r", __FILE__, __LINE__); \
//    }
//
//#define CHECK_TEXTURE_MAGIC(texture, retval) \
//    if (!texture || texture->magic != &texture_magic) { \
//    	xil_printf("\t%s:%d in CHECK_TEXTURE_MAGIC !!!ERROR!!!\n\r", __FILE__, __LINE__); \
//    }
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_CreateTextureFromSurface() \n\r", __FILE__, __LINE__);
...
SDL_UpdateTexture(SDL_Texture * texture, const SDL_Rect * rect,
                  const void *pixels, int pitch)
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t%s:%d in SDL_UpdateTexture \n\r", __FILE__, __LINE__);
	SDL_Rect real_rect;
    ...
    if (rect) {
    	if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: SDL_IntersectRect \n\r");
    ...
    if (real_rect.w == 0 || real_rect.h == 0) {
    	if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: nothing to do. \n\r");
        return 0;  /* nothing to do. */
#if SDL_HAVE_YUV
    } else if (texture->yuv) {
    	if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: SDL_UpdateTextureYUV \n\r");
        return SDL_UpdateTextureYUV(texture, &real_rect, pixels, pitch);
#endif
    } else if (texture->native) {
    	if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: SDL_UpdateTextureNative \n\r");
        return SDL_UpdateTextureNative(texture, &real_rect, pixels, pitch);
    } else {
        SDL_Renderer *renderer = texture->renderer;
        if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: FlushRenderCommandsIfTextureNeeded \n\r");
        if (FlushRenderCommandsIfTextureNeeded(texture) < 0) {
            return -1;
        }
        if(XIL_PRINTF_EN == 1) xil_printf("SDL_UpdateTexture: renderer->UpdateTexture \n\r");
...
int SDL_UpdateYUVTexture(SDL_Texture * texture, const SDL_Rect * rect,
                         ...
                         const Uint8 *Vplane, int Vpitch)
{
	if(XIL_PRINTF_EN == 1) xil_printf("[%s:%d] in SDL_UpdateYUVTexture() \n\r", __FILE__, __LINE__);
#if SDL_HAVE_YUV
    SDL_Renderer *renderer;
    SDL_Rect real_rect;
               ...
               const SDL_Rect * srcrect, const SDL_Rect * dstrect)
{
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("SDL_RenderCopy: SDL_RenderCopyF \n\r");
                ...
                const SDL_Rect * srcrect, const SDL_FRect * dstrect)
{
    ...
    if (renderer != texture->renderer) {
    	if(XIL_PRINTF_EN == 1) xil_printf("\t[%s:%d] in if (renderer != texture->renderer) ERROR \n\r", __FILE__, __LINE__);
        return SDL_SetError("Texture was not created with this renderer");
    }
    ...
    if (srcrect) {
        if (!SDL_IntersectRect(srcrect, &real_srcrect, &real_srcrect)) {
        	if(XIL_PRINTF_EN == 1) xil_printf("\t 1 [%s:%d] in SDL_INTERSECTRECT return 0\n\r", __FILE__, __LINE__);
            return 0;
        }
    }
    ...
    if (dstrect) {
        if (!SDL_HasIntersectionF(dstrect, &real_dstrect)) {
        	if(XIL_PRINTF_EN == 1) xil_printf("\t 2 [%s:%d] in SDL_INTERSECTRECT \n\r", __FILE__, __LINE__);
            return 0;
        }
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("SDL_RenderCopy: SDL_RenderCopyF: inside \n\r");
...
SDL_RenderPresent(SDL_Renderer * renderer)
{
	...
	if(XIL_PRINTF_EN == 1) xil_printf("\t 0.0 [%s:%d] SDL_RenderPresent: CHECK_RENDERER_MAGIC \n\r", __FILE__, __LINE__);
	CHECK_RENDERER_MAGIC(renderer, );

    if(XIL_PRINTF_EN == 1) xil_printf("\t 0.1 [%s:%d] SDL_RenderPresent: FlushRenderCommands \n\r", __FILE__, __LINE__);
    FlushRenderCommands(renderer);  /* time to send everything to the GPU! */
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t 0.2 [%s:%d] SDL_RenderPresent: renderer->RenderPresent(renderer); \n\r", __FILE__, __LINE__);
...
{
	if(XIL_PRINTF_EN == 1) xil_printf("\t\t[%s:%d] in SDL_DestroyRenderer\n\r", __FILE__, __LINE__);
	SDL_RenderCommand *cmd;

	if(renderer==NULL) xil_printf("\t\t_STATE 0.0\n\r");

    CHECK_RENDERER_MAGIC(renderer, );

    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 0.1\n\r");
    SDL_DelEventWatch(SDL_RendererEventWatch, renderer);

    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 0.2\n\r");
    if (renderer->render_commands_tail != NULL) {
    	if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 0.2.1\n\r");
        renderer->render_commands_tail->next = renderer->render_commands_pool;
        cmd = renderer->render_commands;
    } else {
    	if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 0.2.2\n\r");
        cmd = renderer->render_commands_pool;
    }

    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 1\n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 2\n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 3\n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t_STATE 4\n\r");
    ...
    if(XIL_PRINTF_EN == 1) xil_printf("\t\t____[%s:%d] out SDL_DestroyRenderer\n\r", __FILE__, __LINE__);
}
